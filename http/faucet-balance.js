...
export class FaucetBalance extends BaseElement {
  ...
  render() {
    const available = this.balances[this.network]?.available
      ? HTN(this.balances[this.network].available) + " HTN"
      : "---";
    const pending = this.balances[this.network]?.pending ? HTN(this.balances[this.network].pending) + " HTN" : null;
    ...
    const blocksSinceLastUpdate = this.blocksSinceLastUpdate
      ? (this.blocksSinceLastUpdate < 10 ? "" : "") + this.blocksSinceLastUpdate.toFixed(1)
      : "--.-";
